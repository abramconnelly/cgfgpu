...
#define cleanup_err() do { ret=1; cgf4_show_help(stderr); goto cgf_cleanup; } while (0);
#define cleanup_fail() do { ret=-2; goto cgf_cleanup; } while (0);
#define cleanup_ok() do { ret=0; cgf4_show_help(stderr); goto cgf_cleanup; } while (0);
#define cleanup() do { cgf4_show_help(stderr); goto cgf_cleanup; } while (0);
...
void cgf4_show_help(FILE *fp) {
  ...
  fprintf(fp, "\n");
  fprintf(fp, "Usage:\n");
  fprintf(fp, "   cgf [-H][-b tilepath][-e tilepath][-i ifn][-o ofn][-h][-v][-V][ifn]\n");
  fprintf(fp, "   cgf [-x] \n");
  fprintf(fp, "\n");
  fprintf(fp, "   -x                         select cgfx (GPU) mode. ignores cgf4 options.\n");
  fprintf(fp, "\n");
  fprintf(fp, "CGF4 options (default mode):\n");
...
int cgf4_main (int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "4Hb:e:i:o:Ct:T:L:U:hvVAZRIqmp:P:s:S:YF:", long_options, &option_index))!=-1) switch (opt) {
    case 0:
  ...
  if (cgf_opt.show_help) { cgf4_show_help(stdout); goto cgf_cleanup; }
  ...
  //
    ...
    if ((ifp=fopen(cgf_opt.ifn.c_str(), "rb"))==NULL) { perror(cgf_opt.ifn.c_str()); cleanup_err(); }
    cgf = cgf_read(ifp);
    if (!cgf) {
      printf("CGF read error.  Is %s a valid CGFv4 file?\n", cgf_opt.ifn.c_str());
      cleanup_fail();
    }
    ...
    if (cgf_opt.band_ifn.size()>0) {
      if ((cgf_opt.band_ifp=fopen(cgf_opt.band_ifn.c_str(), "rb"))==NULL) { perror(cgf_opt.band_ifn.c_str()); cleanup_err(); }
    } else {
  ...
  //
    ...
    //if ((ifp=fopen(cgf_opt.ifn, "rb"))==NULL) { perror(cgf_opt.ifn); cleanup_err(); }
  ...
  //
    ...
    if (!(ifp = fopen(cgf_opt.ifns[0].c_str(), "rb"))) {
    ...
    if (!(ifp = fopen(cgf_opt.ifns[1].c_str(), "rb"))) {
